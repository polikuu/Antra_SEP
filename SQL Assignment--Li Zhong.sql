 ...
 left join Purchasing.Suppliers on (Application.People.PersonID = Purchasing.Suppliers.PrimaryContactPersonID or Application.People.PersonID = Purchasing.Suppliers.AlternateContactPersonID)
 left join sales.customers on (application.people.personID = sales.customers.PrimaryContactPersonID or application.people.personID = sales.customers.AlternateContactPersonID)
 ...
 where application.people.PhoneNumber = sales.customers.PhoneNumber and sales.Customers.BuyingGroupID is not null;
 ...
 (
    select sales.orders.CustomerID from sales.orders
    where sales.orders.orderdate > '2016-01-01'
  );
 ...
 select warehouse.StockItems.StockItemName, sum(Purchasing.PurchaseOrderLines.OrderedOuters) as total from Purchasing.PurchaseOrderLines
 inner join Purchasing.PurchaseOrders on Purchasing.PurchaseOrders.PurchaseOrderID = Purchasing.PurchaseOrderLines.PurchaseOrderID
 ...
 select distinct (warehouse.StockItems.StockItemName) from warehouse.StockItems
 ...
 select asp.StateProvinceName, avg(datediff(day, so.OrderDate, si.ConfirmedDeliveryTime)) as 'average days' from Application.StateProvinces as asp
 ...
 select asp.StateProvinceName, month(so.orderdate) as 'month', avg(datediff(day, so.OrderDate, si.ConfirmedDeliveryTime)) as 'average days' from Application.StateProvinces as asp
 inner join Application.Cities as ac on asp.StateProvinceID = ac.StateProvinceID
 inner join sales.Customers as sc on sc.DeliveryCityID = ac.CityID
 inner join sales.Orders as so on so.CustomerID = sc.CustomerID
 inner join sales.Invoices as si on si.OrderID = so.OrderID
 ...
 select 
 ws.StockItemName,
 sum(so.pickedquantity) as 'sold',
 sum(pol.receivedouters) as 'purchase'
 ...
 acou.CountryName, 
 sc.CustomerName,
 ap.fullname as 'Contact Person', 
 ap.PhoneNumber as 'Contact Person Phone'
 ...
 wsg.StockGroupID,
 wsg.StockGroupName,
 sum (cast (pol.receivedouters as bigint) ) as 'Purchased',
 sum (cast(so.pickedquantity as bigint)) as 'Sold',
 sum (cast(pol.receivedouters as bigint)) - sum (cast(so.pickedquantity as bigint)) as 'Remining Stock'
...
/*14.	List of Cities in the US and the stock item that the city got the most deliveries in 2016. If the city did not purchase any stock items in 2016, print “No Sales”.*/
 ...
 with cte (cityID, cityname, orderdate, description, total) as 
  ...
  (select ac.CityID, ac.CityName, so.OrderDate, sol.description,
      case 
 ...
 select cte.cityID, max(cte.cityname) as city,
   case 
 ...
 /*15.	List any orders that had more than one delivery attempt (located in invoice table).*/
   ...
   */
   ...
   select (len(ReturnedDeliveryData)-len(replace(ReturnedDeliveryData, 'DeliveryAttempt', '')))/len('DeliveryAttempt') as DeliveryAttempTimes
   from sales.Invoices
  ...
  /*17.	Total quantity of stock items sold in 2015, group by country of manufacturing.*/
   ...
   select json_value(Warehouse.StockItems.CustomFields, '$.CountryOfManufacture') as Country, sum(sales.OrderLines.PickedQuantity) as Total_quantity
 ...
 (select wsg.StockGroupName, year(so.OrderDate) as year1, sol.PickedQuantity 
 from sales.orders as so
 inner join sales.orderlines as sol on sol.orderID = so.OrderID
 ...
 (sum(PickedQuantity)
  ...
  /*21.	Create a new table called ods.Orders. Create a stored procedure, with proper error handling and transactions, that input is a date; 
		...
		create proc date_orders @date datetime
		as 
	    Begin 	
			set nocount on;		
				begin transaction;
	    ...
	    select * into ods.orders
		from (
	
		select sol.orderID, so.OrderDate, so.CustomerID, sum((sol.UnitPrice * sol.PickedQuantity) * (1+sol.TaxRate/100)) as total from sales.Orders as so
...
select tt.* into gg from (
 ...
 'Novelty Items' as [Novelty_Items], 'Furry Footwear' as [Furry_Footwear], Mugs as [Mugs], 'Computing Novelties' as [Computing Novelties],
 Toys as [Toys] 
 ...
 (sum(PickedQuantity)
  for 
  groups in ([T-Shirts], [USB Novelties], [Packaging Materials], [Clothing], [Novelty Items], [Furry Footwear], [Mugs], [Computing Novelties], [Toys])) as pivottable) as mm
  
  for json path; 

  /*26.	Revisit your answer in (19). Convert the result into an XML string and save it to the server using TSQL FOR XML PATH.*/

  declare @xmlStock as xml = (
  select Year_sold as [Year], 'T-Shirts' as [T-Shirts], 'USB Novelties' as [USB_Novelties], 'Packaging Materials' as [Packaging_Materials], Clothing as [Chlothing],
 ...
 (sum(PickedQuantity)
  ...
  declare @xmlStockChar as varchar(max) = cast (@xmlStock as varchar(max));
